...
void i2c_byte_write_frame(u8 sa,u8 mr,u8 data)
{
	...
	if(I2STAT!=0x08)
	{
		uart1_tx_string("Error:start condition\r\n");
		//flag=-1;
		goto exit;
	}
	...
	if(I2STAT==0x30)
	{
		uart1_tx_string("Error:memory address\r\n");
		//flag=-1;
		goto exit;
	}
...
unsigned char i2c_byte_read_frame(u8 sa,u8 mr)
{
	...
	if(I2STAT!=0x10)
	{
		uart1_tx_string("Error:restart condition\r\n");
		goto exit;
	}
